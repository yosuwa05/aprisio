  ...
  .get(
    ...
    async ({ query }) => {
      try {
        ...
        // Add user joined info if userId is provided and valid
        if (userId && userId !== "undefined" && ObjectId.isValid(userId)) {
          aggregationPipeline.push(
            {
              $lookup: {
                from: "usersubtopics",
                let: { subTopicId: "$subTopic._id" },  // Don't use $toObjectId here
                pipeline: [
                  {
                    $match: {
                      $expr: {
          ...
          // Add default joined field if userId is not provided or invalid
          aggregationPipeline.push({
            $addFields: {
              "subTopic.joined": false
            }
          });
        }
        ...
        aggregationPipeline.push(
          {
            $lookup: {
              from: "groups",
              let: {
                subTopicId: "$subTopic._id",  // Don't use $toObjectId here
