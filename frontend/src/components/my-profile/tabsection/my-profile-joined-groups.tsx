...
import MyProfileGroupCard from "../my-profile-groupcard";

export const MyProfileJoinedGroups = () => {
          ...
          page.groups.length > 0 ? (
            page.groups.map((group: any, postIndex: number) => (
              <React.Fragment key={group?._id}>
                <MyProfileGroupCard group={group?.group} />
              </React.Fragment>
            ))
          ) : (
            <p
              key={`${pageIndex}-no-groups`}
              className='text-gray-500 text-xs font-semibold'></p>
          )
        )
      ) : (
