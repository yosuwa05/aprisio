...
import MyProfileTopicCard from "../my-profile-topic-card";
    ...
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = 1 }) => {
        const res = await _axios.get(
          `/myprofile/joined-communities/?userId=${user?.id}&page=${pageParam}&limit=${limit}`
        );
        return res?.data;
      },
      initialPageParam: 1,
      getNextPageParam: (lastPage, allPages) => {
        ...
        return lastPage?.communities?.length === limit ? nextPage : undefined;
      },
    });
  console.log(data);
    ...
    <div className='flex flex-col gap-6 items-center p-1 lg:p-4'>
      {isLoading ? (
        ...
        data.pages.map((page, pageIndex) =>
          page?.communities?.length > 0 ? (
            page?.communities?.map((topic: any, postIndex: number) => (
              <React.Fragment key={topic?._id}>
                <MyProfileTopicCard communities={topic} />
              </React.Fragment>
            ))
          ) : (
            <p
