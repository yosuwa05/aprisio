		...
		<div class="relative grid grid-cols-2 items-center gap-2">
			...
			>
				<Select.Trigger class="capitalize">
					{filter ? (filter === 'true' ? 'Approved' : 'Pending') : 'Filter'}
				</Select.Trigger>
				<Select.Content>
					<Select.Group>
			...
			<Table.Body>
				{#each $query.data?.events || [] as event, i}
					<Table.Row>
						...
						<Table.Cell>
							<button onclick={() => $approvevent.mutate({ id: event._id })}>
								<Badge
									class={`ml-2 ${event.isApprovedByAdmin ? 'bg-green-500 text-white hover:bg-green-700' : 'bg-red-500 text-white hover:bg-red-700'}`}
									variant="secondary"
								>
									{event?.isApprovedByAdmin ? 'Approved' : 'Pending'}
								</Badge>
							</button>
						</Table.Cell>
					</Table.Row>
