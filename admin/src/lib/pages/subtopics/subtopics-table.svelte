<script lang="ts">
	...
	const query = createQuery({
		...
		queryFn: () => fetchSubtopics(limit, page, search)
	});

	const deleteMutation = createMutation({
		mutationFn: ({ id, permanent }: { id: string; permanent: boolean }) =>
			_axios.delete(`/subtopic/${id}?permanent=${permanent}`),
		onSuccess({ data }) {
	...
	<div class="text-maintext font-karla mx-auto mt-6">
		...
		<Table.Root class="">
			...
			{:else if $query?.data?.total === 0}
				<TableCaption class="w-full text-center text-xs">No Subtopics Found!</TableCaption>
			{/if}
			<Table.Header>
				<Table.Row class="">
			...
			<Table.Body>
				{#each $query.data?.subtopics || [] as topic, i}
					<Table.Row>
						...
						<Table.Cell class="flex gap-2">
							...
							<button
								onclick={() => (
									(modelOpen = true), ($subTopicsStore = { ...$subTopicsStore, id: topic._id })
								)}
							>
		...
		<Dialog.Root open={modelOpen} onOpenChange={(e) => (modelOpen = e)}>
			<Dialog.Content class="p-6">
				<Dialog.Header class="text-center">
					<Dialog.Title class="text-center">Do you want to delete this Subtopic ?</Dialog.Title>
				</Dialog.Header>

				<div class="flex justify-around gap-4">
					<Button
						class="w-full bg-red-500 font-bold text-white hover:bg-red-400"
						onclick={() => $deleteMutation.mutate({ id: $subTopicsStore.id, permanent: true })}
						>Yes</Button
					>
